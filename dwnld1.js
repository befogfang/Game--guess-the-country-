...
function ranOptButton(){
    ...
    if(rNo%2 == 0){
        let bifCoA = option2.innerText=`${cLValue[rNo]}`; 
        option1.innerText= valForOpt1 ;
        let ifCoA = bifCoA;
        messageToShown(bifCoA);

    }
    else{
        let belseCoA =option1.innerText=`${cLValue[rNo]}`;
       let elseCoA = belseCoA;
        option2.innerText= valForOpt1 ;

        messageToShown(belseCoA);
...
function messageToShown(ans) {
    
    this.addEventListener("click",(ans)=>{
        // console.log(this.el);
        messages.innerText=this.ans;
    })
  }